...
/// Generates bounded image and video previews outside Flutter's UI isolate.
final class ThumbnailHandler: NSObject {
    ...
    private func generateThumbnail(
        ...
        switch mediaType {
        case "image":
            generateImageThumbnail(path: path, maxPixelSize: maxPixelSize, result: result)
        case "video":
    ...
    private func generateVideoThumbnail(
        ...
        let requestedSeconds = durationSeconds.isFinite && durationSeconds > 0
            ? min(max(durationSeconds * 0.1, 0), max(durationSeconds - 0.05, 0))
            : 0
