...
class MediaLoaded extends MediaState {
  ...
  final Set<MediaType> activeMediaTypes;
  ...
  MediaLoaded copyWith({
    ...
    Set<MediaType>? activeMediaTypes,
  }) {
    return MediaLoaded(
      ...
      activeMediaTypes: activeMediaTypes ?? this.activeMediaTypes,
    );
  }
}
...
class MediaViewModel extends StateNotifier<MediaState> {
  ...
  static final Set<MediaType> _defaultMediaTypes =
      Set<MediaType>.from(MediaType.values);
  Set<MediaType> _activeMediaTypes =
      Set<MediaType>.from(_defaultMediaTypes);
  ...
  Set<MediaType> get activeMediaTypes =>
      Set<MediaType>.unmodifiable(_activeMediaTypes);
  ...
  Future<void> loadMedia() async {
    ...
    try {
      ...
      if (media.isEmpty) {
        ...
        _clearSelectionInternal();
        state = MediaLoaded(
          media: _applyActiveFilters(_cachedMedia),
          searchQuery: '',
          ...
          activeMediaTypes: _activeMediaTypes,
        );
      }
    } catch (e) {
      final totalTime = DateTime.now().difference(loadStartTime);
  ...
  void searchMedia(String query) {
    if (state case MediaLoaded(
      ...
      final results = _applyActiveFilters(
        _cachedMedia,
        searchQuery: query,
      );
  ...
  List<MediaEntity> _applyTypeFilter(List<MediaEntity> media) {
    if (_activeMediaTypes.length == _defaultMediaTypes.length &&
        _activeMediaTypes.containsAll(_defaultMediaTypes)) {
      return List<MediaEntity>.from(media);
    }
    final allowedTypes = _activeMediaTypes;
    return media
        .where((item) => allowedTypes.contains(item.type))
        .toList(growable: false);
  }

  List<MediaEntity> _applyActiveFilters(
    List<MediaEntity> media, {
    String searchQuery = '',
  }) {
    final typeFiltered = _applyTypeFilter(media);
    final favoritesFiltered = _applyFavoritesFilter(typeFiltered);
    return _applySearch(favoritesFiltered, searchQuery);
  }
  ...
  /// Updates which media types are visible in the grid and reapplies filters.
  void updateMediaTypeFilters(Set<MediaType> mediaTypes) {
    final sanitized = mediaTypes.isEmpty
        ? Set<MediaType>.from(_defaultMediaTypes)
        : Set<MediaType>.from(mediaTypes);

    if (setEquals(sanitized, _activeMediaTypes)) {
      return;
    }

    _activeMediaTypes = sanitized;
    _emitLoadedStateFromCache();
  }
