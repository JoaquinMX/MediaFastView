...
void main() {
  test(
    'merges tags by name, unions assignments, and re-links by live id',
    () async {
      const rootPath = '/lib';
      ...
      final directoryRepository = FakeDirectoryRepository(<DirectoryEntity>[
        root,
      ]);
      ...
      final result = await useCase(
        backup: backup,
        rootMappings: <String, String?>{'/old/library': rootId},
      );
      ...
      final favoriteTypes = favoritesRepository.added
          .map((f) => f.itemType)
          .toList();
      expect(
        favoriteTypes,
        containsAll(<FavoriteItemType>[
  ...
  test('returns an empty result when no manifests are found', () async {
    ...
    final result = await useCase(
      backup: backup,
      rootMappings: <String, String?>{'/lib': root.id},
    );
  ...
  test('prepares exact mappings and leaves moved roots for the user', () async {
    final currentRoot = DirectoryEntity(
      id: 'current',
      path: '/current/photos',
      name: 'photos',
      thumbnailPath: null,
    ...
    final backup = SidecarBackup(
      ...
      roots: const <SidecarBackupRoot>[
        SidecarBackupRoot(
          originalPath: '/current/photos',
          name: 'photos',
          manifestsByRelativeFolder: <String, SidecarManifest>{},
        ),
        SidecarBackupRoot(
          originalPath: '/old/videos',
          name: 'videos',
          manifestsByRelativeFolder: <String, SidecarManifest>{},
        ),
      ],
    ...
    final preparation = await useCase.prepare(backup);

    expect(preparation.automaticRootMappings, <String, String>{
      '/current/photos': currentRoot.id,
    });
    expect(preparation.unmatchedRoots.single.originalPath, '/old/videos');
  });

  test(
    'reports explicitly skipped saved roots without mutating repositories',
    () async {
      final directoryRepository = FakeDirectoryRepository();
      final mediaRepository = FakeMediaRepository();
      final tagRepository = FakeTagRepository();
      final favoritesRepository = FakeFavoritesRepository();
      final useCase = ImportSidecarsUseCase(
      ...
      final result = await useCase(
        backup: backup,
        rootMappings: const <String, String?>{'/old/photos': null},
      );

      expect(result.rootsSkipped, 1);
      expect(mediaRepository.upserted, isEmpty);
      expect(favoritesRepository.added, isEmpty);
