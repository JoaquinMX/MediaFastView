...
void main() {
  ...
  group('DirectoryFilterTree', () {
    ...
    testWidgets('aligns leaf rows with rows that carry an expand chevron',
      ...
      final photos = tester.getTopLeft(
        find.descendant(
          of: find.ancestor(of: find.text('Photos'), matching: find.byType(Row))
              .first,
          matching: find.byType(Checkbox),
        ),
      );
  ...
  group('DirectoryFilterTree columns', () {
    ...
    testWidgets('keeps the roots side by side when the first root is expanded',
        (tester) async {
      await pumpTree(tester, nodes: [_photos(), _downloads()], width: 800);
    ...
    testWidgets('collapsing the promoted root folds the columns back to a row',
        (tester) async {
      await pumpTree(tester, nodes: [_photos()], width: 800);
      expect(find.text('Archive'), findsOneWidget);
